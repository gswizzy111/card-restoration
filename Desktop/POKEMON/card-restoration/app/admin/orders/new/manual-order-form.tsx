...
export function ManualOrderForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (pricePerCardCents <= 0) { setError("Enter a price per card."); return; }
    ...
    <form onSubmit={handleSubmit} className="flex flex-col gap-6 max-w-xl">
      ...
      <div className="bg-white rounded-xl border border-border p-6 flex flex-col gap-4">
        ...
        <div>
          ...
          <input className={input} value={name} onChange={(e) => setName(e.target.value)} required placeholder="John Smith" />
        </div>
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className={label}>Email *</label>
            <input className={input} type="email" value={email} onChange={(e) => setEmail(e.target.value)} required placeholder="john@example.com" />
          </div>
          <div>
            <label className={label}>Phone *</label>
            <input className={input} value={phone} onChange={(e) => setPhone(e.target.value)} required placeholder="(555) 555-5555" />
          </div>
        </div>
      </div>
